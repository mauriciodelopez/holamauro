...
float calculate_creche_fee(float father_salary, float mother_salary, int days_per_week, int hours_per_day, int num_children)
{
    ...
    // Calculate the base fee (monthly, not per hour)
    float base_fee = total_salary * coef_caf;

    // Calculate the fee based on days and hours
    int weeks_per_month = 4; // Average weeks per month
    float fee_per_hour = base_fee / (5 * 8); // Assuming standard 5 days, 8 hours per day
    float total_fee = fee_per_hour * days_per_week * hours_per_day * weeks_per_month;
