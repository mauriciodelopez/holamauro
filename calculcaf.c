...
float calculate_creche_fee(float father_salary, float mother_salary, int days_per_week, int hours_per_day, int num_children)
{
    ...
    // Calculate the hourly cost for one child
    float cost_per_hour = (total_salary * coef_caf) / 100;

    // Calculate the monthly fee
    int weeks_per_month = 4; // Average weeks per month
    float total_hours = days_per_week * hours_per_day * weeks_per_month;
    float total_fee = cost_per_hour * total_hours;
